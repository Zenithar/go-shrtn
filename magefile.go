...
func Build() {
	banner := figure.NewFigure("Shrtn", "", true)
	banner.Print()
...
func (Bin) shrtn() error {
	return goBuild("go.zenithar.org/shrtn/cli/shrtn", "shrtn")
}
...
func goBuild(packageName, out string) error {
	...
	varsSetByLinker := map[string]string{
		"go.zenithar.org/shrtn/internal/version.Version":   tag(),
		"go.zenithar.org/shrtn/internal/version.Revision":  hash(),
		"go.zenithar.org/shrtn/internal/version.Branch":    branch(),
		"go.zenithar.org/shrtn/internal/version.BuildUser": os.Getenv("USER"),
		"go.zenithar.org/shrtn/internal/version.BuildDate": time.Now().Format(time.RFC3339),
		"go.zenithar.org/shrtn/internal/version.GoVersion": runtime.Version(),
	}
